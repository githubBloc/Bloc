					...
					echo "Return Code: " . $_FILES["photos"]["error"][$name] . "<br>";
						...
						{		
		 					 mkdir("../uploads/".$row["id"], 0777);
									...
									$select = $counter."Pic";
									$results2 = mysql_query("SELECT LogoPic FROM PremiumPictures WHERE BizId='$BizId'");
									$row2 = mysql_fetch_array($results2);
									
									if($counter==1 && $row2['LogoPic']=="" || $row2['LogoPic']==null)
									{
										mysql_query("INSERT INTO PremiumPictures (BizId, $select) VALUES('$BizId','$PicPath')") or die(mysql_error()); 
									}
									 else {	mysql_query("UPDATE PremiumPictures SET $select='$PicPath' WHERE BizId='$BizId'") or die(mysql_error()); }
													
									 echo "<li><img id='PictureGal' src='".$PicPath."'></li>";
